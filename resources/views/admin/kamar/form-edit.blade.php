    ...
    <div class="min-h-screen w-full bg-gradient-to-br from-slate-50 to-slate-100/50 pt-0 pb-8" x-data="kamarForm()">
        <div class="w-full">
            <div class="mb-8 flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-slate-900">Edit Kamar</h1>
            ...
            <form id="form-kamar" action="{{ route('kamar.update', $kamar->id) }}" method="POST" enctype="multipart/form-data" class="overflow-hidden rounded-2xl bg-white shadow-xl ring-1 ring-slate-200">
                @csrf
                @method('PUT')
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        <div class="group">
                            ...
                            <div class="relative">
                                ...
                                <input id="kode_kamar" value="{{ $kamar->kode_kamar }}" name="kode_kamar" type="text" required placeholder="Contoh: A-101, B-205" x-model="formState.kode_kamar"
                                    @blur="formState.touched.kode_kamar = true" @input="checkForChanges"
                        ...
                        <div class="group">
                            ...
                            <div class="relative">
                                ...
                                <input id="harga" value="{{ $kamar->harga }}" name="harga" type="text" inputmode="numeric" placeholder="1.500.000" x-model="formState.harga"
                                    @input="handleHargaInput; checkForChanges()" @blur="formState.touched.harga = true"
                ...
                <div class="space-y-6 px-8 py-8">
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <select id="tipe" name="tipe" x-model="formState.tipe" @change="updateFasilitasByTipe; checkForChanges()" @blur="formState.touched.tipe = true"
                                    class="w-full rounded-xl border-2 border-slate-200 bg-white py-3 pl-11 pr-4 text-sm font-medium text-slate-900 placeholder:text-slate-400 transition-all focus:border-cyan-500 focus:outline-none focus:ring-4 focus:ring-cyan-500/10">
                                    <option value="">- Pilih Tipe -</option>
                                    <option value="Standard" @selected($kamar->tipe === 'Standard')>Standard</option>
                                    <option value="Medium" @selected($kamar->tipe === 'Medium')>Medium</option>
                                    <option value="Exclusive" @selected($kamar->tipe === 'Exclusive')>Exclusive</option>
                                </select>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input id="lebar" value="{{ $kamar->lebar }}" name="lebar" type="number" min="0" step="0.1" placeholder="Contoh: 12.5" x-model="formState.lebar"
                                    @blur="formState.touched.lebar = true" @input="checkForChanges"
                                    class="w-full rounded-xl border-2 border-slate-200 bg-white py-3 pl-11 pr-4 text-sm font-medium text-slate-900 placeholder:text-slate-400 transition-all focus:border-cyan-500 focus:outline-none focus:ring-4 focus:ring-cyan-500/10" />
                    ...
                    <div>
                        ...
                        <textarea id="deskripsi" name="deskripsi" rows="4" placeholder="Deskripsikan fasilitas, kondisi, dan keunggulan kamar ini..." x-model="formState.deskripsi"
                            @blur="formState.touched.deskripsi = true" @input="checkForChanges"
                            class="w-full rounded-xl border-2 border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 transition-all focus:border-cyan-500 focus:outline-none focus:ring-4 focus:ring-cyan-500/10">{{ $kamar->deskripsi }}</textarea>
                    ...
                    <div>
                        ...
                        <div id="drop-zone" @dragover.prevent="isDragging = true" @dragleave.prevent="isDragging = false" @drop.prevent="handleDrop"
                            ...
                            <input type="file" name="gambar" accept="image/*" id="gambar-input" class="hidden" @change="handleFileSelect; formState.touched.gambar = true; checkForChanges()">

                            <div x-show="!formState.gambar && !formState.existingImage" class="flex flex-col items-center justify-center px-6 py-12 text-center">
                            ...
                            <!-- Gambar lama -->
                            <div x-cloak x-show="formState.existingImage && !formState.gambar" class="relative">
                                <img src="{{ Storage::url($kamar->gambar) }}" alt="{{ $kamar->gambar }}" class="h-64 w-full object-cover" />
                                <div class="absolute inset-0 bg-gradient-to-t from-slate-900/60 to-transparent"></div>
                                ...
                                <div class="absolute bottom-4 left-4 right-4">
                                    <p class="text-sm font-semibold text-white">Gambar saat ini</p>
                                    <p class="text-xs text-white/80">Klik ikon sampah untuk menghapus gambar ini</p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        <template x-for="(fasilitas, index) in fasilitasList" :key="index">
                            <label
                                ...
                                <input type="checkbox" name="fasilitas[]" :value="fasilitas" :checked="formState.fasilitas.includes(fasilitas)"
                                    @change="toggleFasilitas(fasilitas); checkForChanges()"
                ...
                <div class="flex flex-col gap-4 border-t border-slate-200 bg-slate-50 px-8 py-6 sm:flex-row sm:items-center sm:justify-between">
                    <p class="text-sm text-slate-600">
                        ...
                        <span x-show="!hasChanges" class="text-slate-500">Tidak ada perubahan yang dilakukan</span>
                        <span x-show="hasChanges" class="text-amber-600">Ada perubahan yang belum disimpan</span>
                    </p>
                    <div class="flex gap-3">
                        ...
                        <button type="submit" :disabled="!isFormValid || !hasChanges"
                            ...
                            <span x-text="hasChanges ? 'Simpan Perubahan' : 'Tidak Ada Perubahan'"></span>
                        </button>
                    </div>
                </div>
            </form>
            ...
            return {
                ...
                get hasChanges() {
                    ...
                    const mainFieldsChanged =
                        ...
                        this.formState.tipe !== this.originalState.tipe ||
                        this.formState.lebar !== this.originalState.lebar ||
                        this.formState.deskripsi !== this.originalState.deskripsi ||
                ...
                toggleFasilitas(fasilitas) {
                    ...
                    if (index > -1) {
                        this.formState.fasilitas.splice(index, 1);
                    } else {
