        ...
        <div class="max-w-6xl mx-auto">
            ...
            <div class="mb-12" x-data="{ activeTab: 0, isZoomed: false }">
                ...
                <div class="relative rounded-2xl overflow-hidden shadow-lg mb-4 bg-neutral-200 transition-smooth"
                    @click="isZoomed = true">
                    <img :src="isZoomed ? '{{ Storage::url($kamar->gambar) }}' : '{{ Storage::url($kamar->gambar) }}'"
                        alt="{{ $kamar->tipe }}"
                        class="w-full h-96 sm:h-[500px] object-cover cursor-zoom-in transition-smooth hover:scale-105"
                        x-show="!isZoomed">

                    <!-- Zoom Modal -->
                    <div class="fixed inset-0 bg-black/95 z-50 flex items-center justify-center p-4 cursor-zoom-out"
                        @click="isZoomed = false" x-show="isZoomed" x-transition.opacity>
                        <img src="{{ Storage::url($kamar->gambar) }}" alt="{{ $kamar->tipe }}"
                            class="max-w-4xl max-h-screen object-contain">
                        <button
                            class="absolute top-4 right-4 text-white text-4xl font-light hover:opacity-70 transition-smooth">&times;</button>
                    </div>

                    <!-- Badge Status -->
                    <div class="absolute top-4 left-4 flex items-center gap-2">
                        <span
                            class="px-4 py-2 bg-success/90 text-white rounded-full text-xs font-semibold backdrop-blur-sm">
                            ✓ Tersedia
                        </span>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-4 sm:grid-cols-6 gap-2">
                        @foreach ($kamar->galeri as $index => $gambar)
                            <button @click="activeTab = {{ $index }}"
                                class="relative rounded-lg overflow-hidden border-2 transition-smooth group"
                                :class="activeTab === {{ $index }} ? 'border-primary' :
                                    'border-neutral-200 hover:border-primary'">
                                <img src="{{ $gambar }}" alt="Gallery"
                                    class="w-full h-20 object-cover group-hover:scale-110 transition-smooth">
                            </button>
                        @endforeach
                    </div>
                @endif
                ...
                <div class="lg:col-span-2">
                    ...
                    <div class="mb-10">
                        <div class="flex items-start justify-between mb-4">
                            <div>
                                <span
                                    class="inline-block px-3 py-1 bg-primary/10 text-primary rounded-full text-xs font-semibold mb-3">
                                    {{ $kamar->tipe }}
                                </span>
                    ...
                    <section class="mb-12">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div class="flex items-start gap-3">
                                <svg class="w-6 h-6 text-primary flex-shrink-0 mt-1" fill="currentColor"
                                    viewBox="0 0 20 20">
                    ...
                    <section class="mb-12">
                        ...
                        <div class="glass-card rounded-xl p-6 space-y-4">
                            ...
                            <div class="flex justify-between py-3 border-b border-neutral-200">
                                ...
                                <span
                                    class="text-primary font-bold text-lg">Rp{{ number_format($kamar->harga, 0, ',', '.') }}</span>
                            </div>
                            <div class="flex justify-between py-3">
                                <span class="text-neutral-600 font-medium">Status</span>
                                <span
                                    class="px-3 py-1 bg-success/10 text-success rounded-full text-xs font-semibold">Tersedia</span>
                            </div>
                        </div>
                    </section>
                    ...
                    <section class="mb-12">
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                            @foreach ($kamar->detailKamar as $item)
                                <div
                                    class="glass-card rounded-lg p-4 flex items-center gap-3 hover:shadow-md transition-smooth">
                                        ...
                                        @switch($item->fasilitas)
                                            @case('Kasur & Bantal')
                                                <svg class="w-6 h-6 text-primary" fill="none" stroke="currentColor"
                                                    ...
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                        d="M3 9v12h18V9a3 3 0 00-3-3H6a3 3 0 00-3 3zm9 3H6m12 0h-6m-6 4h12" />
                                                </svg>
                                            @break
                                            ...
                                            @case('Meja dan Kursi')
                                                <svg class="w-6 h-6 text-primary" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                        d="M3 9h18v2H3V9zm2 4h2v6H5v-6zm12 0h2v6h-2v-6zM7 9v6m10-6v6" />
                                                </svg>
                                            @break
                                            ...
                                            @case('K. Mandi Dalam')
                                                <svg class="w-6 h-6 text-primary" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                        d="M5 3v18h14V3H5zm2 2h10v2H7V5zm0 4h10v8H7V9z" />
                                                </svg>
                                            @break
                                            ...
                                            @case('Dapur Pribadi')
                                                <svg class="w-6 h-6 text-primary" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                        d="M3 6h18M3 12h18M3 18h18M5 6v12M19 6v12" />
                                                </svg>
                                            @break
