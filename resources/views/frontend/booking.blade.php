...
@section('frontend-main')
    ...
    <div class="min-h-screen bg-gradient-to-b from-neutral-50 to-white">
        ...
        <!-- Filter Section -->
        <section
            class="top-0 z-40 bg-gradient-to-br from-white via-blue-50/30 to-white backdrop-blur-xl border-b border-neutral-200/50 shadow-lg transition-all">
            <div class="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-8">
                <div class="space-y-6">
                    ...
                    <div class="relative group">
                        ...
                        <div
                            class="absolute left-5 top-1/2 -translate-y-1/2 transition-transform duration-300 group-focus-within:scale-110 group-focus-within:rotate-12">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                        ...
                        <div class="group">
                            ...
                            <div class="relative">
                                ...
                                <svg class="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-blue-500 pointer-events-none"
                        ...
                        <div class="group">
                            ...
                            <div class="relative">
                                ...
                                <svg class="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-green-500 pointer-events-none"
                                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M19 9l-7 7-7-7"></path>
                                </svg>
                        ...
                        <div class="group">
                            ...
                            <div
                                class="px-5 py-3.5 bg-white/80 backdrop-blur-sm border-2 border-neutral-200 rounded-xl shadow-sm hover:shadow-md transition-all duration-300">
                                <div class="flex items-center justify-between mb-3">
                                    ...
                                    <span
                                        ...
                                        Rp <span
                                            id="priceValue">{{ number_format($kamar->max('harga') ?? 10000000, 0, ',', '.') }}</span>
                                    </span>
                                    <span class="text-xs font-medium text-neutral-500">Rp
                                        {{ number_format($kamar->max('harga') ?? 10000000, 0, ',', '.') }}</span>
                                </div>
                                <input type="range" id="priceFilter" min="0"
                                    max="{{ $kamar->max('harga') ?? 10000000 }}" step="100000"
                                    value="{{ $kamar->max('harga') ?? 10000000 }}"
            ...
            <div class="max-w-7xl mx-auto">
                ...
                <div class="mb-8">
                    <p class="text-neutral-600">
                        Menampilkan <span class="font-semibold text-neutral-900"
                            id="resultsCount">{{ $kamar->count() }}</span> dari
                ...
                <div id="roomsGrid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @forelse($kamar as $index => $item)
                        <div class="room-card group relative bg-white rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500"
                            data-tipe="{{ $item->tipe }}" data-status="{{ $item->status }}"
                            data-harga="{{ $item->harga }}" data-kode="{{ $item->kode_kamar }}"
                            data-deskripsi="{{ $item->deskripsi }}">

                            <!-- Image Container -->
                            <div class="relative h-[500px] overflow-hidden">
                                <img src="{{ Storage::url($item->gambar) ?? asset('assets/image/dummy/standard.jpg') }}"
                                    alt="{{ $item->kode_kamar }}"
                                    class="room-image w-full h-full object-cover transition-transform duration-700 ease-out">

                                <!-- Favorite Icon -->
                                <button
                                    class="favorite-btn absolute top-4 right-4 z-20 w-10 h-10 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center hover:bg-white/90">
                                    <svg class="w-5 h-5 text-white group-hover:text-red-500 transition-colors"
                                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <div
                                    class="default-info absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black/60 to-transparent">
                                ...
                                <div
                                    class="hover-content absolute inset-0 pointer-events-none group-hover:pointer-events-auto">
                                    <!-- Blur Overlay -->
                                                    ...
                                                    <div>
                                                        ...
                                                        <div class="flex flex-wrap gap-1.5">
                                                            @foreach ($item->detailKamar->take(3) as $detail)
                                                                <span
                                                                    class="inline-block px-2 py-1 bg-white/20 backdrop-blur-sm text-white text-xs rounded-md">
                                                                    {{ $detail->fasilitas }}
                                                                </span>
                                                            @endforeach
                    ...
                    @empty
                        ...
                        <div class="col-span-full flex flex-col items-center justify-center py-20 px-4">
                            <svg class="w-20 h-20 text-neutral-300 mb-4" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
