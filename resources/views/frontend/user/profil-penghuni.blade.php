            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
                <div>
                    <div class="flex items-center gap-3 mb-3">
                        <a href="{{ route('dashboard-penghuni') }}" class="inline-flex items-center text-sm font-medium text-indigo-600 hover:text-indigo-800 transition-colors">
                ...
                <div class="flex items-center gap-4 p-3">
                    ...
                    @endif
                    <div class="text-left">
                        <p class="font-semibold text-slate-900 text-sm">{{ auth()->user()->name ?? 'Penghuni Kos' }}</p>
                        <p class="text-xs text-slate-500 capitalize">{{ auth()->user()->role ?? 'Penghuni' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-8">
                    ...
                    <div class="bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden transition-all hover:shadow-md">
                        ...
                        <div class="p-6">
                            <form action="{{ route('profil-penghuni.update') }}" method="POST" class="space-y-5">
                                @csrf
                                @method('PUT')

                                <div>
                                    <label for="name" class="block text-sm font-medium text-slate-800 mb-2">Nama Lengkap</label>
                                    <input type="text" name="name" id="name" value="{{ old('name', auth()->user()->name ?? 'Penghuni Kos') }}"
                                ...
                                <div>
                                    ...
                                    <input type="email" name="email" id="email" value="{{ old('email', auth()->user()->email ?? 'penghuni@kos.com') }}"
                                        class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                                    @error('email')
                                        <p class="mt-1.5 text-sm text-red-600 font-medium">{{ $message }}</p>
                                    @enderror
                    ...
                    <div class="bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden transition-all hover:shadow-md">
                        ...
                        <div class="p-6">
                            <form action="{{ route('profil-penghuni.update-avatar') }}" method="POST" enctype="multipart/form-data" class="space-y-6" x-data="{ preview: null, hasFile: false }">
                                @csrf
                                @method('PUT')
                                    ...
                                    <div class="space-y-4">
                                        <div>
                                            ...
                                            <input type="file" name="avatar" id="avatar" accept="image/*"
                                                ...
                                                @change="preview = URL.createObjectURL($event.target.files[0]); hasFile = $event.target.files.length > 0">
                                        </div>

                                        <!-- Preview Baru -->
                                        <div x-cloak x-show="preview" class="mt-3">
                                ...
                                <button type="submit" x-cloak
                                    class="px-5 py-3 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white font-medium rounded-xl shadow-sm hover:shadow-md transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                                    :disabled="!hasFile">
                ...
                <div class="space-y-8">
                    ...
                    <div class="bg-gradient-to-b from-amber-50 to-orange-50 border border-amber-200 rounded-2xl shadow-sm overflow-hidden">
                        ...
                        <div class="p-6">
                            <form action="{{ route('profil-penghuni.update-password') }}" method="POST" class="space-y-5">
                                @csrf
                                @method('PUT')
